...
import type { RegisterPayload } from "@/services/apiService";
...
export default function RegisterForm() {
  ...
  const registerMutation = useMutation({
    mutationFn: (values: RegisterPayload) => apiService.register(values),
    onSuccess: () => {
      router.push("/login");
    },
    onError: (error: any) => {
    ...
    <Card className="w-full max-w-md border-white/10 bg-[#0f172a]/80 backdrop-blur-xl">
      <CardHeader className="space-y-1">
        ...
        <CardDescription className="text-slate-400">
          Get started with EcoTrack for free
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
        ...
        <button
          type="button"
          onClick={() =>
            toast.info("Google signup is an upcoming feature. Please use email registration for now.")
          }
          ...
          <div className="space-y-2 text-xs">
            {[
              ...
              <button
                ...
                type="button"
                onClick={() => {
                  form.setValue("name", c.role + " Demo");
